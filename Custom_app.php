<?php
/*
Plugin Name: After Product Protection Plan
Plugin URI: https://www.afterinc.com/contact/
Description: Plugin allows merchants to sell After Plan’s product protection plans for wide variety of products.
Version: 3.7.1
Author: After Inc.
...
/* apply filter for warranty and sync hook*/
  add_action( 'pre_get_posts', array($this,'apply_warranty_product_filters' ));
   ...
   add_filter('cron_schedules', array($this, 'add_cron_interval_for_order_sync'));
  ...
  function add_js_customer() {
      ...
      wp_enqueue_script('contract_admin_js', plugin_dir_url(__FILE__) . 'assets/js/afterinc-customer.js');
      ...
      wp_enqueue_style( 'add_customer_css', plugins_url('/assets/css/afterinc-customer.css', __FILE__));
      wp_localize_script('jquery', 'ajax', array(
          'url' => admin_url('admin-ajax.php'),
          'nonce' => wp_create_nonce('After##Inc@20'),
      ));
  }
 ...
 /* add JS file for admin into the plugin*/
  function add_js_admin() {
      ...
      wp_enqueue_style('style_css', plugin_dir_url(__FILE__) . 'assets/css/admin/afterinc-style.css');
  ...
  function customplugin_menu() {
   add_menu_page("Custom Plugin", "After Plans","manage_options", "After Plans", array( $this, 'configuration_form' ), 'dashicons-format-aside');
   
   add_submenu_page( 'After Plans', 'setting', 'Settings', 'manage_options', 'After Plans', array( $this, 'configuration_form' ), 'dashicons-format-aside');

    add_submenu_page( 'After Plans', 'Bulk Product Update', 'Bulk Product Update', 'manage_options', 'Bulk-Product-Update', array( $this, 'Submenu_function' ), 'dashicons-format-aside');
  }
...
/*Display custom warrenty filed data form database*/
    ...
    if( $old_status != 'publish' && $new_status == 'publish' && !empty($post->ID) && in_array( $post->post_type, array( 'product') )) {
      ...
      $result = $this->wpdb->get_results( $this->wpdb->prepare( "SELECT * FROM app_configurations WHERE id = %d", 1 ) );
       ...
       if(!empty($result) && $result[0]->sync_type == 1 && $result[0]->configuration_property == 1 && $result[0]->warranty_status == 1){
        ...
        $product_json = json_encode($dbData);
         ...
         $base_url = AfterInc::API_BASE_URL;
      ...
      $this->wpdb->query( $this->wpdb->prepare( "INSERT INTO $table_name ( post_id, meta_key, meta_value)
         VALUES ( %d, %s, %s )", $productId, '_sync', 'Synced'));
		  plugin_log(['statuscode'=>$response_status,'response'=>$response,'request'=>$product_json]);
  ...
  function action_woocommerce_update_product( $product_id ) {
          ...
          if(($responseStatus == 200)){ 
             ...
             $dbData['price'] = ((float) $newprice * 100);
              ...
              $dbData['category'] = $category;
               $dbData['description'] = ($product->get_description() != '') ? $product->get_description() : 'N/A';
             $dbData['model'] = ($product->get_sku() !='') ? $product->get_sku() : $product_id;
    ...
    function add_cron_interval_for_order_sync($schedules) {
   ...
   function my_custom_5_minute_function() {
     plugin_log("This is testing for every five minute test");
...
<!-- Modal -->
  ...
  function pricechange(){
    ...
    if(empty($result)){
      ...
      VALUES ( %d, %d, %d, %d, %s)",$current_user_id,$productId,$warrantyId,$warrantyPrice,$planId));
        ...
        //echo json_encode(['totalPrice'=>$totalPrice,'currency_symbol'=>$this->get_currency_symbol()]);
  ...
  function saveconfig(){
     ...
     if ( !wp_verify_nonce($_POST['nonce'], 'After##Inc@20') ){ 
      ...
      if ( $loop->have_posts() ): while ( $loop->have_posts() ): $loop->the_post();
          ...
          $sku = ($product->get_sku() !='') ? $product->get_sku() : $product_id;
           ...
           $model= ($product->get_sku() !='') ? $product->get_sku() : $product_id;
            $category = ($product->get_categories() != '') ? $product->get_categories() : '';
           $category = strip_tags($category);
           $category = strtolower($category);
        ...
        if ($response_status == 202) {
          ...
          plugin_log(['statuscode'=>$response_status,'response'=>$response,'request'=>$productJson]); 
        } 
       ...
       if(!empty($getContracts)){
        ...
        <table bgcolor="#d4e2ea" style="margin: 0 auto; width: 600px; background-color: #d4e2ea; padding: 25px;">
            ...
            <tr style="padding-top: 20px; margin-bottom: 10px;">
                <td>
                   <p>Your customer has been cancelled ' . $planDesc . ' Please initiate a refund of $' . $subtotal . ' through '.$siteTitle.'. <br />
...
function add_item_meta($item_data, $cart_item)
 ...
 // sync order to the quickcover
  ...
  $product_id = $item['product_id'];
	...
	if(!empty($get_configuration_property)){
...
if(isset($result[0]->contract_status) && $result[0]->contract_status == '200' && $result[0]->refund_amount !=''){
  ...
  $refundamtqty = $result[0]->refund_amount * $item['quantity'];
   ...
   $cancelButton = "<button class='btn' data-orderId=".$productOrderId." data-lineItemID =".$lineItemId." disabled=true style='cursor: not-allowed; pointer-events: none; color: #a7aaad; padding: 5px 10px; font-size: 13px; border: 1px solid #dcdcde; background: #f6f7f7; font-family: sans-serif;'> Cancel ".$pnamecustomer." </button>";
    ...
    $results = $this->wpdb->get_results( $this->wpdb->prepare( "SELECT * FROM quickcover_contract WHERE order_id=".$productOrderId) );
    
   if (!empty($results)){
   $cancelButton = "<button class='cancelContract' id=".$item_quantity." style='display: block; padding: 5px 10px; font-size: 13px; border: none; background: #009688; font-family: sans-serif; float: right; margin-right: 30%' data-orderId=".$productOrderId." data-lineItemID=".$lineItemId." data-cancelby='customer'> Cancel ".$pnamecustomer."</button>";
  ...
  ?>
   <style type="text/css">
    ...
    if (in_array($productOrderId, $order_id)){
       ...
       echo '<p><button type="button" class="btn button-primary" data-orderId="'.esc_attr($productOrderId).'" data-lineItemID ="'.esc_attr($lineItemId).'" disabled=true style="cursor: not-allowed;">Due to technical issues, the contract has not been created yet.<br>Please try again later.</button></p>';
                ...
                if(isset($orderData->contracts)){
                  ...
                  plugin_log(['contract_status'=>$response_status,'response'=>$response,'request'=>$orderList]);
                   ...
                   $message = "<html>
                    ...
                    // plugin_log('createdTimeDb::'.$value->created_at);
                    // $dtt = date('Y-m-d H:i:s',strtotime($value->created_at));
                    // plugin_log($dtt ." & ". date('Y-m-d H:i:s'));
                    // $diffItmer = round($this->differenceInHours($dtt,date('Y-m-d H:i:s')),2);
                    // plugin_log("diffminute::".$diffItmer);
